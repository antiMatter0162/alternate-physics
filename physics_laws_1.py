...
def acceleration_dependent_velocity(acceleration, dt, initial_velocity, v_threshold):
    ...
    if initial_velocity < v_threshold:
        ...
        acceleration = acceleration * (initial_velocity/v_threshold) * gamma
    else:
        velocity = initial_velocity - acceleration * dt * (initial_velocity/v_threshold) * gamma
        acceleration = - acceleration * (initial_velocity/v_threshold) * gamma
        v_threshold = v_threshold * 1.01
    return velocity, v_threshold
...
def get_velocity(acceleration, dt, initial_velocity, v_threshold, n):
    ...
    time = []
    for i in range(n):
        velocity, v_threshold = acceleration_dependent_velocity(acceleration, dt, initial_velocity, v_threshold)
